...
abstract class XiRouter
{
    ...
    public function _getMenus()
    {
        if($this->_menus ===null){
        	$site = new JSite();
			$this->_menus 	= $site->getMenu()->getItems('component_id',JComponentHelper::getComponent($this->_component)->id);
	}
    ...
    public function build( &$query )
    {
            ...
            $condition  = array_shift($keys);
            if(isset($query['Itemid']) && ($condition === 'Itemid') ){
                // if item-id exists, then pick the var from menu and put into query, if not exist already
                $site = new JSite();
     			$item = $site->getMenu()->getItem($query['Itemid']);
