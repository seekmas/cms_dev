...
if(!class_exists('XiAbstractModelBaseAdapt')) {
	if(interface_exists('JModel')) {
		abstract class XiAbstractModelBaseAdapt extends JModelLegacy {}
	} else {
		class XiAbstractModelBaseAdapt extends JModel {}
	}
}

abstract class XiAbstractModelBase extends XiAbstractModelBaseAdapt
{
	...
	public function _populateGenericFilters(Array &$filters=array())
	{
		...
		//fetch the cross filter from model itself
		$crossFilters = array_keys($this->filterMatchOpeartor);
		
		foreach ($crossFilters as $crossFilter)
		{
			$vars[$crossFilter] = null;	
		}
